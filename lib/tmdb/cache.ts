...
import { pickTrailerKey, pickOfficialTrailerForLanguage } from "./client";
...
export async function cacheMovie(detail: TmdbMovieDetail): Promise<void> {
  try {
    ...
    const { error } = await admin.from("movies_cache").upsert({
      ...
      trailer_key_tr: pickOfficialTrailerForLanguage(detail.videos?.results ?? [], "tr"),
      trailer_key_fr: pickOfficialTrailerForLanguage(detail.videos?.results ?? [], "fr"),
...
export interface ReelCandidate {
  ...
  trailerKey: string;
  trailerKeyTr: string | null;
  trailerKeyFr: string | null;
  overview: string;
...
export async function getReelCandidatePool(
  ...
  const { data, error } = await supabase
    .from("movies_cache")
    .select(
      "tmdb_id, title, poster_path, backdrop_path, trailer_key, trailer_key_tr, trailer_key_fr, overview, release_year, external_rating, genre_ids, director, cast_members, original_language, popularity",
    )
  ...
  return (data ?? [])
    ...
    .map((row) => ({
      ...
      trailerKeyTr: row.trailer_key_tr,
      trailerKeyFr: row.trailer_key_fr,
      overview: row.overview ?? "",
